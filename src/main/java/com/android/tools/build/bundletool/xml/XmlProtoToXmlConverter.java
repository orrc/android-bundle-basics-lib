...
public final class XmlProtoToXmlConverter {
  ...
  private Node createXmlNode(XmlProtoNode protoNode, Document xmlFactory) {
    ...
    // Add the namespaces.
    List<XmlProtoNamespace> namespaces =
        protoElement.getNamespaceDeclarations().collect(toList());
    ...
    for (XmlProtoNode child : protoElement.getChildren().collect(toList())) {
  ...
  private String getPrefixForNamespace(String attrNamespaceUri) {
    Deque<String> prefixes =
        namespaceUriToPrefix.computeIfAbsent(
            attrNamespaceUri,
            uri -> createDequeWithElement(getCommonPrefix(uri).orElseGet(this::createNewPrefix)));
